...
class MusicReleaseTests(TestCase):
    """
    test music release creation/translation
    """

    @classmethod
    def setUpTestData(cls):
        """
        Create a music release object
        """
        music_release = MusicRelease.objects.create(
            title="Music Release",
            description="This is a music release.",
            live=True,
            image="music/music_release.jpg",
        )
        music_release.set_current_language("ja")
        music_release.title = "リリース"
        music_release.description = "これはリリースです。"
        music_release.live = True
        music_release.image = "music/music_release.jpg"
        music_release.save()

    @classmethod
    def tearDownClass(cls):
        """
        Delete the music release object
        """
        MusicRelease.objects.get(translations__title="Music Release").delete()
        assert MusicRelease.objects.count() == 0, MusicRelease.objects.all()

    def test_default_music_release(self):
        """
        Test that the music release was created
        """
        music_release = MusicRelease.objects.get(translations__title="Music Release")
        self.assertEqual(music_release.title, "Music Release")
        self.assertEqual(music_release.description, "This is a music release.")
        self.assertEqual(music_release.live, True)
        self.assertEqual(music_release.image, "music/music_release.jpg")

    def test_english_music_release(self):
        """
        Test that the English music release was created
        """
        music_release = MusicRelease.objects.get(translations__title="Music Release")
        music_release.set_current_language("en")
        self.assertEqual(music_release.title, "Music Release")
        self.assertEqual(music_release.description, "This is a music release.")
        self.assertEqual(music_release.live, True)
        self.assertEqual(music_release.image, "music/music_release.jpg")

    def test_japanese_music_release(self):
        """
        Test that the Japanese music release was created
        """
        music_release = MusicRelease.objects.get(translations__title="リリース")
        music_release.set_current_language("ja")
        self.assertEqual(music_release.title, "リリース")
        self.assertEqual(music_release.description, "これはリリースです。")
        self.assertEqual(music_release.live, True)
        self.assertEqual(music_release.image, "music/music_release.jpg")